...
export function drawMurchik(ctx, { x, y, u, time, facing = 1, activity = 'calm' }) {
  ...
  // Rounded body and contrasting chest follow the original Compose layout.
  oval(.5,.735,.29,.255,fur.base); oval(.5,.755,.148,.185,fur.belly);
  // Tabby stripes on body.
  for (const yy of [.62,.68,.74,.80]) line([[.27,yy],[.73,yy]],.026,fur.shade);
  // Paws and head.
  oval(.36,.83,.105,.17,fur.base); oval(.64,.83,.105,.17,fur.base);
  ...
  oval(.5,.47,.285,.255,fur.base); oval(.5,.57,.18,.105,fur.belly);
  ...
  // Cheek and paw details are deliberately separate from the tabby body bands.
  for (const side of [-1,1]) { oval(.5+side*.18,.61,.045,.025,'#e9a7a0'); for (const px of [-.025,.025]) oval(.5+side*.14+px,.87,.012,.008,fur.patch); }
